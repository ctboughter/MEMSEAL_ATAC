...
counts <- Read10X_h5(filename = "out_y109/outs/filtered_peak_bc_matrix.h5")
metadata <- read.csv(
  file = "out_y109/outs/singlecell.csv",
  header = TRUE,
  row.names = 1
)
...
chrom_assay <- CreateChromatinAssay(
  counts = counts,
  sep = c(":", "-"),
  fragments = 'out_y109/outs/fragments.tsv.gz',
  min.cells = 10,
  min.features = 200
)
...
pdf(file = "out_y109/outs/qc1.pdf", width = 8,height = 4)
...
pdf(file = "out_y109/outs/qc2.pdf", width = 8,height = 4)
...
#pdf(file = "out_y109/outs/qc3.pdf", width = 8,height = 4)
#FragmentHistogram(object = data, group.by = 'nucleosome_group')
# dev.off does the printing, I guess
#dev.off()

pdf(file = "out_y109/outs/qc3.pdf", width = 16,height = 4)
VlnPlot(
  object = data,
...
write.csv(mat, "out_y109/outs/mat_109.csv")
write.csv(gene_meta, "out_y109/outs/gene_meta_109.csv")
...
pdf(file = "out_y109/outs/umap.pdf", width = 8,height = 8)
...
pdf(file = "out_y109/outs/expression.pdf", width = 12,height = 8)
FeaturePlot(
  object = data,
...
pdf(file = "out_y109/outs/coverage.pdf", width = 16,height = 8)
CoveragePlot(
  object = data,
...
pdf(file = "out_y109/outs/coverage2.pdf", width = 16,height = 8)
CoveragePlot(
  object = data,
  region = "Cd3d",
  annotation = FALSE,
...
SaveH5Seurat(data, 'out_y109/outs/atac_process109.h5', overwrite = FALSE, verbose = TRUE)

Convert("out_y109/outs/atac_process109.h5.h5seurat",dest="h5ad")